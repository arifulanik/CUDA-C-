...
//Section 1 L 16
//Sum array with timing
//Idea: In CUDA we generally compare the performance CPU with GPU implementaion. So, we have to measure the execution time. 
//formula: clock start = clock(), work load, clock end = clock() then diff = end-start
//time = diff/clock_per_sec
...
{
	int size = 1 << 25;
	//Trial and error method to find out best grid configuration
	//int block_size = 128; //Total GPU time : 0.126

	int block_size = 256; //Total GPU time : 0.105 ------->Best 
	//int block_size = 512; //Total GPU time : 0.110

	//int block_size = 1024; //Total GPU time : 0.114 
...
//Most of the time consumed in the memeory transfer, mainly from host to device
// Only kernel execution time is very less than CPU
//Except for time we have to use "trial and error" method to find out the best block configuration and choose the best configuration based on the execution time
